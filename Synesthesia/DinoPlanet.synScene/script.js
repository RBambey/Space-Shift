...
//   - altMin/altMax: driven by min_height / max_height controls, offset by
//     terrainHeight() so clearance holds over the rolling hills, not just
//     over a flat y = 0 plane

var TREE_SCALE = 8.0;

function fract(x) { return x - Math.floor(x); }

// Mirrors terrainHash()/terrainNoise()/terrainHeight() in main.glsl exactly —
// keep the two in sync if either changes.
function terrainHash(px, py) {
    px = fract(px * 127.1);
    py = fract(py * 311.7);
    var d = px * (px + 17.5) + py * (py + 17.5);
    px = fract(px + d);
    py = fract(py + d);
    return fract(px * py);
}
...
function terrainNoise(x, y) {
    ...
    fx = fx * fx * (3.0 - 2.0 * fx);
    fy = fy * fy * (3.0 - 2.0 * fy);
    var a = terrainHash(ix, iy)     + (terrainHash(ix + 1, iy)     - terrainHash(ix, iy))     * fx;
    var b = terrainHash(ix, iy + 1) + (terrainHash(ix + 1, iy + 1) - terrainHash(ix, iy + 1)) * fx;
    return a + (b - a) * fy;
}

function terrainHeight(worldX, worldZ) {
    var px = worldX / TREE_SCALE, pz = worldZ / TREE_SCALE;
    var h = terrainNoise(px * 0.06, pz * 0.06) * 1.0;
    h += terrainNoise(px * 0.15 + 11.3, pz * 0.15 + 11.3) * 0.4;
    return h * TREE_SCALE;
}
...
function update(dt) {
    cam.update(dt,
        ...
        { recenterY: function(x, z) { return terrainHeight(x, z) + min_height + 1.5; },
          altMin:    function(x, z) { return terrainHeight(x, z) + min_height; },
          altMax:    function(x, z) { return terrainHeight(x, z) + max_height; } }
    );
}
