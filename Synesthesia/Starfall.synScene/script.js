// Starfall — Flight physics + starfall time accumulator
// Camera module copied from Ocean Planet (RBambey).
// starfallTime integrates fall_speed*dt each frame — avoids TIME precision issues.
...
var cam = (function(opts) {
    ...
    // Start looking straight down into the falling stars.
    // camFwd = world -Y; camRight = world +X; camUp = world +Z (screen top = forward).
    var camRight = [1, 0, 0];
    var camUp    = [0, 0, 1];
    var camFwd   = [0, -1, 0];
    ...
    function update(dt, controls, callbacks) {
        ...
        // Forward movement — no altitude clamp (free space)
        camFwd = normalize3(camFwd);
...
var starfallTime = 0.0;

function setup() {
    cam.setup();
    setUniform("starfall_time", 0.0);
}
...
function update(dt) {
    cam.update(dt,
        ...
        { recenterY: function() { return 0.0; } }
    );

    // Integrate fall_speed smoothly — avoids TIME*speed teleportation and precision loss
    starfallTime = (starfallTime + (fall_speed || 0.0) * dt) % 100.0;
    setUniform("starfall_time", starfallTime);
}
