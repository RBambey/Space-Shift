// flightCamera.js — shared basis-vector flight camera for Synesthesia scenes
// Based on ocean_planet.synScene by RBambey
//
// Usage:
//   var cam = require('flightCamera')({ startY: 5.0 });
//   function setup() { cam.setup(); }
//   function update(dt) {
//       cam.update(dt,
//           { pitch, roll_rate, yaw_rate, barrel_roll, fly_speed, recenter },
//           { recenterY: function(x,z) { return 5.0; },
//             altMin:    function(x,z) { return 2.0; },
//             altMax:    function(x,z) { return 40.0; },
//             afterUpdate: function() { /* scene-specific uniforms */ } });
//   }
//
// opts:
//   startY           — initial altitude (default 5.0; 0 is valid)
//   snapXOnRecenter  — pull camX → 0 during recenter (default false)
//
// callbacks:
//   recenterY(x,z)  — target altitude during recenter; omit to skip Y snap
//   altMin(x,z)     — floor clamp after each frame; omit for no clamping
//   altMax(x,z)     — ceiling clamp after each frame; omit for no clamping
//   afterUpdate()   — called after pushUniforms each frame

module.exports = function createFlightCamera(opts) {
    opts = opts || {};
    ...
    function update(dt, controls, callbacks) {
        ...
        if (recenterActive) {
            ...
            var doneOrientation = Math.abs(camFwd[1]) < 0.01 && Math.abs(camUp[1] - 1.0) < 0.01;
            var doneX = !snapX || Math.abs(camX) < 0.1;
            var doneY = targetY == null || Math.abs(camY - targetY) < 0.1;

            if (doneOrientation && doneX && doneY) {
