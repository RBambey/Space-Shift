// Retro 70s Gas Giant — Flight physics + color helpers
...
function setHSVColor(uniformName, hue, sat, val) {
    var px = Math.abs(fract(hue + 1  ) * 6 - 3);
    var py = Math.abs(fract(hue + 2/3) * 6 - 3);
    var pz = Math.abs(fract(hue + 1/3) * 6 - 3);
...
function setNormalizedVec3(uniformName, x, y, z) {
    var il = 1 / Math.sqrt(x*x + y*y + z*z);
...
function update(dt) {
    cam.update(dt,
        ...
        { recenterY: function()    { return min_height + 2.0; },
          ...
          afterUpdate: function() {
              ...
              setUniform("GG", bh_position.x, bh_position.y, 1000, 400);
          }
        }
    );
}
