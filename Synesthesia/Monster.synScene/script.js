// Monster — Flight physics
// Based on Ocean Planet by RBambey
// Free 6-DOF flight through a parade of towering eldritch juggernauts.
...
var cam = (function(opts) {
    ...
    var camRight = [1, 0, 0];
    var camUp    = [0, 0.974, 0.227];
    var camFwd   = [0, -0.227, 0.974];
    ...
    function update(dt, controls, callbacks) {
        ...
        // Recenter — levels orientation and snaps back to monster eye level
        if (recenter > 0.5 && prevRecenter < 0.5) recenterActive = true;
        prevRecenter = recenter;
        if (recenterActive) {
            var pull    = 3.0 * dt;
            var targetY = callbacks.recenterY ? callbacks.recenterY() : null;
            ...
            if (doneOri && doneY) recenterActive = false;
        }
        ...
        // Forward movement — free 3D flight through the monster field
        camFwd = normalize3(camFwd);
        camX += camFwd[0] * fly_speed * dt;
        camY += camFwd[1] * fly_speed * dt;
        camZ += camFwd[2] * fly_speed * dt;

        // Loose altitude bounds — stay within the monster zone
        var altMin = callbacks.altMin ? callbacks.altMin() : -Infinity;
        var altMax = callbacks.altMax ? callbacks.altMax() :  Infinity;
...
function update(dt) {
    cam.update(dt,
        ...
        { recenterY: function() { return 8.0; },
          altMin:    function() { return min_height; },
          altMax:    function() { return max_height; } }
    );
}
