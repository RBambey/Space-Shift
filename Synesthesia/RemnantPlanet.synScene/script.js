// Remnant Planet — Flight physics
// Adapted from Ocean Planet / Forest Planet by RBambey
//
// Differences from Forest Planet:
//   - startZ: -3.0  (begins just outside the fractal, looking in)
//   - No altitude limits — fly freely through 3D fractal space
//   - fly_speed default 0.3 (fractal world is ~3 units across)
...
var cam = (function(opts) {
    ...
    function update(dt, controls, callbacks) {
        ...
        // Recenter — levels orientation only, no altitude target
        ...
        if (recenterActive) {
            ...
            if (doneOri) recenterActive = false;
        }
        ...
        // Forward movement — no altitude clamping (free 3D flight)
        camFwd = normalize3(camFwd);
...
})({ startX: 0.0, startY: 0.0, startZ: -3.0 });
...
function update(dt) {
    cam.update(dt,
        ...
        // No altitude callbacks — free 3D flight through the fractal
    );
}
